...
export const getConjunctions = async() => {
    try {
        var requestOptions = {
            method: 'GET',
            redirect: 'follow'
        ...
        const load = await axios("https://celestrak.org/SOCRATES/search-results.php?IDENT=NAME&NAME_TEXT1=&NAME_TEXT2=&ORDER=MAXPROB&MAX=10", requestOptions)
        const loadedData = await load.data;
        ...
        for (let item = 0; item < ((table.length - 11) / 13); item++) {
            ...
            async function getTLE(formData) {
                try {
                    const res = await axios("http://celestrak.org/SOCRATES/conjunction.php", {
                        "headers": {
                    ...
                    const tleDataTemp = await res.data;
